...
class Fighter:
    ...
    def attack(self, target):
        ...
        damage = libtcod.random_get_int(0, 1+self.modifier(self.strength), bdamage)
...
def place_objects(room):
    ...
    item_chances = {}
    item_chances['gold'] = 10 #always shows up  
    ...
    for i in range(num_items):
        ...
        if not is_blocked(x, y):
            ...
            if choice == 'heal':
                ...
                item_component = Item(stacks=True, count=1, use_function=cast_heal, pickup_sound=SFX_POTIONPICKUP, use_sound=SFX_POTIONUSE)
...
def new_game():
    ...
    fighter_component = Fighter(hp=100, ac=12, strength=10, dexterity=14, constitution=10, 
...
def play_game():
    global key, mouse, player_dijkstra, gold_dijkstra, sound_dijkstra, scent_map
    ...
    while not libtcod.console_is_window_closed():
        #libtcod.sys_wait_for_event(libtcod.EVENT_KEY_RELEASE | libtcod.EVENT_MOUSE_RELEASE, key, mouse, True)
        #render the screen
        render_all()
        ...
        libtcod.sys_wait_for_event(libtcod.EVENT_KEY_PRESS | libtcod.EVENT_MOUSE_RELEASE, key, mouse, True)
